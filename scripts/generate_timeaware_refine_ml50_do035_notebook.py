...
TIMEAWARE_NDCG10_OUTPUT = "/content/drive/MyDrive/ai-projects/time-aware-behavior-prediction/outputs/sasrec_timeaware_refine_ml50_do035_ndcg10"
TIMEAWARE_NDCG5_OUTPUT = "/content/drive/MyDrive/ai-projects/time-aware-behavior-prediction/outputs/sasrec_timeaware_refine_ml50_do035_ndcg5"
...
NDCG10_NEW_RUNS = [
    ("timeaware_refine_ml50_do035_b8_s42", dict(seed=42, boundaries="60,600,3600,86400")),
    ("timeaware_refine_ml50_do035_b8_s2024", dict(seed=2024, boundaries="60,600,3600,86400")),
    ("timeaware_refine_ml50_do035_b8_s7", dict(seed=7, boundaries="60,600,3600,86400")),
    ("timeaware_refine_ml50_do035_b9_s42", dict(seed=42, boundaries="60,600,3600,86400,604800")),
    ("timeaware_refine_ml50_do035_b9_s2024", dict(seed=2024, boundaries="60,600,3600,86400,604800")),
    ("timeaware_refine_ml50_do035_b9_s7", dict(seed=7, boundaries="60,600,3600,86400,604800")),
]
...
def comparison_section(label: str, baseline_output_dir_var: str, timeaware_output_dir_var: str, baseline_runs: list[str], timeaware_runs: list[str], primary_valid_col: str, primary_test_col: str) -> list[dict]:
...
cells = [
    ...
    md_cell("## Check planned time-aware runs\n\nOnly train runs that are still missing.\n"),
    code_cell(
        "planned_ndcg10 = [\n" +
        "".join([f"    '{name}',\n" for name, _ in NDCG10_NEW_RUNS]) +
        "]\n"
        "planned_ndcg5 = [\n" +
        "".join([f"    '{name}',\n" for name, _ in NDCG5_NEW_RUNS]) +
        "]\n\n"
        "for label, output_dir, run_names in [\n"
...
cells.append(md_cell("## Train time-aware runs for `NDCG@5`\n\nRun these cells only if the corresponding run directory does not already exist.\n"))
...
output_path = Path("notebooks") / "sasrec_timeaware_bpi2012_colab_train_03_260513.ipynb"
